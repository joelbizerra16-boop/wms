...
def contexto_minuta_tabela(request, filtros):
    ...
    total_ms = round((time.perf_counter() - inicio) * 1000, 2)
    _log_tempo_minuta('MINUTA_QUERY_MS', total_ms, etapa='lista', sql_ms=consulta_ms, serial_ms=serializacao_ms, registros=len(linhas), busca=filtros.get('busca') or filtros.get('romaneio') or '', page=current_page, historico=historico)
    return {
        'linhas': linhas,
        'filtros': filtros,
...
def render_minuta_tabela_partial(request, filtros=None):
    ...
    total_ms = round((time.perf_counter() - inicio) * 1000, 2)
    _log_tempo_minuta('MINUTA_RENDER_MS', total_ms, busca=filtros.get('busca') or filtros.get('romaneio') or '')
    return response
