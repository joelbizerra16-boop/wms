...
def _status_base_nf(nf, ultima_conferencia):
    itens_nf = list(nf.itens.all())
    if not itens_nf or ultima_conferencia is None:
        return NotaFiscal.Status.PENDENTE

    conferencia_itens = {item.produto_id: item for item in ultima_conferencia.itens.all()}
...
def atualizar_status_nf(nf):
    ...
    status_base = _status_base_nf(nf, ultima_conferencia)
    ...
    possui_bloqueio_conferencia = bool(ultima_conferencia and ultima_conferencia.status == Conferencia.Status.DIVERGENCIA)
    possui_liberacao_conferencia = bool(
        ultima_conferencia and ultima_conferencia.status in {Conferencia.Status.LIBERADO_COM_RESTRICAO, Conferencia.Status.CONCLUIDO_COM_RESTRICAO}
    )
