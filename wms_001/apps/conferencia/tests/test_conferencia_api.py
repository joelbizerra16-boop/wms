...
class ConferenciaAPITests(TestCase):
    ...
    def test_lista_nfs_disponiveis_e_inicia_conferencia(self):
        ...
        self.assertEqual(response_inicio.data['progresso']['esperado'], 3.0)
        self.assertEqual(ConferenciaItem.objects.filter(conferencia_id=response_inicio.data['id']).count(), 2)
    ...
    def test_divergencia_gera_retorno_para_separacao(self):
        conferencia_id = self.client.post('/api/conferencia/iniciar/', {'nf_id': self.nf.id}, format='json').data['id']
        item_1 = ConferenciaItem.objects.get(conferencia_id=conferencia_id, produto=self.produto_1)
        ...
        self.client.post('/api/conferencia/bipar/', {'conferencia_id': conferencia_id, 'codigo': 'PRD002'}, format='json')
        ...
        self.assertTrue(Tarefa.objects.filter(rota=self.nf.rota, status=Tarefa.Status.ABERTO).exists())

    def test_nao_permite_iniciar_com_outro_conferente_ativo(self):
