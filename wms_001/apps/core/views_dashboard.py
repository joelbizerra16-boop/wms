...
def _setores_usuario_normalizados(usuario):
    ...
    if not setores and getattr(usuario, 'setor', None) and usuario.setor != Usuario.Setor.NAO_ENCONTRADO:
...
def _usuario_pode_ver_nf_por_setor(usuario, nf):
...
def _build_detalhe_nf_context(request, nf_numero):
    ...
    if nf is None or not _usuario_pode_ver_nf_por_setor(request.user, nf):
        return None
...
def _task_reference_date(item):
    if item.nf_id and item.nf and item.nf.data_emissao:
        return item.nf.data_emissao.date()
    if item.tarefa.nf_id and item.tarefa.nf and item.tarefa.nf.data_emissao:
        return item.tarefa.nf.data_emissao.date()
    return item.tarefa.created_at.date()
...
def _calcular_totais_separacao(itens):
    tarefas_por_id = {}
    for item in itens:
        tarefas_por_id[item.tarefa_id] = item.tarefa
    tarefas = list(tarefas_por_id.values())
    total = len(tarefas)
    separado = sum(1 for tarefa in tarefas if tarefa.status in {Tarefa.Status.CONCLUIDO, Tarefa.Status.CONCLUIDO_COM_RESTRICAO})
    em_execucao = sum(1 for tarefa in tarefas if tarefa.status == Tarefa.Status.EM_EXECUCAO)
    pendente = max(total - separado - em_execucao, 0)
    percentual = 0 if total == 0 else round(separado / total * 100, 2)
    percentual_pendente = 0 if total == 0 else round(pendente / total * 100, 2)
    return {
        'total': total,
        'separado': separado,
        'pendente': pendente,
...
def _filtrar_itens_separacao(itens, date_from, date_to, busca):
    ...
    for item in itens:
        ...
        data_referencia = timezone.localtime(item.tarefa.created_at).date() if item.tarefa.created_at else _task_reference_date(item)
        if data_referencia < date_from or data_referencia > date_to:
            continue
...
def dashboard_separacao(request):
    ...
    tarefas_visiveis = get_tarefas_para_separacao(
        request.user,
        data_inicio=date_from,
        data_fim=date_to,
        busca=busca,
    )
    tarefa_ids_visiveis = [tarefa.get('id') for tarefa in tarefas_visiveis if tarefa.get('id') is not None]
    itens = list(
    ...
    indicadores = _calcular_totais_separacao(itens_filtrados)
    logger.info(
        ...
        {'data_inicial': date_from.isoformat(), 'data_final': date_to.isoformat(), 'busca': busca},
    )
    linhas = []
