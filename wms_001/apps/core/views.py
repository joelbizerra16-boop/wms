...
def dashboard_data(request):
    try:
        ...
        # Usa a mesma regra operacional da tela /conferencia, mas limitada ao dia atual.
        nf_ids_hoje = set(
            tarefas_base.filter(nf__isnull=False).values_list('nf_id', flat=True)
        )
        nfs_prontas_conferencia = [
            nf for nf in listar_nfs_disponiveis(request.user) if nf.get('id') in nf_ids_hoje
        ]
        total_separacao = tarefas_em_separacao.count()
        total_conferencia = len(nfs_prontas_conferencia)
