...
def _dashboard_resumo_payload(request):
    ...
    tarefas = get_tarefas_para_separacao(
        request.user,
    ...
    total = len(tarefas)
    separado = sum(1 for tarefa in tarefas if tarefa.get('status') in {Tarefa.Status.CONCLUIDO, Tarefa.Status.CONCLUIDO_COM_RESTRICAO})
    em_execucao = sum(1 for tarefa in tarefas if tarefa.get('status') == Tarefa.Status.EM_EXECUCAO)
    pendente = max(total - separado - em_execucao, 0)
    aguardando = pendente
    percentual = Decimal('0') if total == 0 else (Decimal(separado) / Decimal(total) * Decimal('100')).quantize(Decimal('0.01'))
