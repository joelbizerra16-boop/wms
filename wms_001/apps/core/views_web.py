...
from apps.nf.services.importador_xml import ImportacaoXMLError, extrair_resumo_nfe_xml, importar_xml_nfe
...
def _obter_conferencia_contexto(nf_id, usuario):
    ...
    conferencia_ativa = nf.conferencias.filter(status=Conferencia.Status.EM_CONFERENCIA, conferente=usuario).first()
...
def confirmar_scan_entradas_web(request):
    ...
    liberadas = 0
    erros = 0
    for entrada in EntradaNF.objects.filter(id__in=ids):
        if entrada.status != EntradaNF.Status.AGUARDANDO:
            continue
        try:
...
def separacao_lista_web(request):
    ...
    contexto = {'tarefas': listar_tarefas_disponiveis(request.user)}
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return _render(request, 'partials/separacao_lista_tabela.html', contexto)
    return _render(request, 'separacao_lista.html', contexto)
